...
angular.module("Uv5kiman")
    .controller("uv5kiSupervCtrl", function ($scope, $interval, $timeout, $serv, $lserv) {
        ...
        function ctrl_ope_is_sync(sync_str) {
            return sync_str && sync_str.startsWith('Sync');
        }
        ...
        ctrl.ext_res_detail = (i) => {
            ...
            detail +=
                '<div class="row">' +
                '<div class="col-md-10 ext-res-popup">' +
                ext_detail_eq_id(i) +
                '</div>' +
                '<div class="col-md-2 ext-res-popup">' +
                ext_detail_eq_st(i) +
                '</div>' +
                '</div>';
            if (i.tipo != 5) {
                detail +=
                    '<div class="row">' +
                    '<div class="col-md-10 ext-res-popup">' + ext_detail_rs_id(i) +
                    '</div>' +
                    '<div class="col-md-2 ext-res-popup">' + ext_detail_rs_st(i) +
                    '</div>' +
                    '</div>';
            }
            detail += '</div>';
